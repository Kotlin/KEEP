...
Adding a primary property to an existing MFVC is roughly analogous to adding a primary property to a data class: it can preserve source and binary compatibility, but changes the semantics.

**Source and binary compatibility.**
Adding a constructor parameter with a default value preserves source compatibility, but does not preserve constructor binary compatibility by itself: already compiled callers still reference the old constructor signature and, for calls using default arguments, its synthetic default bridge.
These entry points must remain available through manually written overloads or version overloads generated by [`@IntroducedAt`](https://github.com/Kotlin/KEEP/blob/main/proposals/KEEP-0431-version-overloading.md).
...
Here `@IntroducedAt` generates a hidden constructor equivalent to the previous `Point(Int)` one, including the default-argument bridge when one is required, and delegates it to the new one.
This holds for both Stage 1 and Stage 2: at Stage 2, Valhalla does not have any special treatment for binary evolution of value classes, so adding a field is compatible once the old constructor ABI has been preserved.
