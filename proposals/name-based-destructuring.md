...
### Future Migration
We are exploring the possibility of making name-based destructuring **the default** for data classes in the future.
While there are no concrete plans yet, this change could be introduced gradually, starting as an experimental feature.
We are also considering how it interacts with other auto-generated functions (e.g. `copy`) in data classes.
Any migration path will be carefully designed to minimize impact.
