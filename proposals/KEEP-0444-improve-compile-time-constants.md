...
Please note that evaluation of `Enum.ordinal` is not supported by design. Unlike `Enum.name`, `Enum.ordinal` is an invisible change to the user of a given enum. If two enum entries change their order, the ordinal values will change undetected by the user. This could easily lead to changes in runtime.
