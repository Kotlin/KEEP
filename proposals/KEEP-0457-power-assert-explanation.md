...
The scope of this KEEP is focused on the details required to enable these APIs to work, and it is unlikely we will be
able to add anything beyond the current scope. But we want to hear your ideas! Shaping the future of these Power-Assert
APIs will require hearing from the community what you all want to create. And that's the most important part: what are
you looking to create? Without an understanding of what your use cases are and what you want to create, we won't be able
to pragmatically guide the evolution of these APIs. While comments and feedback on the current APIs are absolutely
welcome, try to keep the focus of API improvement ideas to use cases. And if you really want to get into the weeds on a
use case idea, consider heading over to the [Kotlin Lang Slack](https://slack-chats.kotlinlang.org/c/power-assert) and
leaving a comment in the `#power-assert` channel!
