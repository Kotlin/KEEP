...
The calendar- and timezone-oblivious operations involve
...
On different platforms, we define different maximum and minimum `Instant`
values:

* JVM:
  `-1000000000-01-01T00:00Z .. +1000000000-12-31T23:59:59.999999999Z`.
* Native, Wasm, and JS:
  `-1000000-01-01T00:00:00Z .. +1000000-12-31T23:59:59.999999999Z`.

Both of these ranges are wide enough to support every realistic need.
...
The `-1000000..1000000` year range is slightly more convenient:
...
On the JVM, we do not limit the range to `+1000000/-1000000` to preserve the
ability to freely convert between `java.time.Instant` and `kotlin.time.Instant`,
and on the other platforms, we use the smaller range.

The discrepancy between ranges on different platforms may be a cause for
concern.
When can we observe a discrepancy between the ranges?
...
* In complex multi-codebase scenarios, it can happen that
  ...
  fail.
  We address this concern separately: see the "Behavior on overflow" section.

###### `Instant.MAX` and `Instant.MIN` become error-prone
...
This decision does not block us, as we can add an annotation like `DelicateApi`
to deter people from using these values as sentinels, but we should keep this
in mind.
...
There are several strategies for dealing with leap seconds:
...
For nanoseconds and microseconds, their ranges are smaller than our
`Instant` ranges (at least +/- `1_000_000` years).
For milliseconds, the range is wider than the Native and JS range, but still
smaller than the JVM range (+/- `1_000_000_000` years).

Whenever an `Instant` is too big to fit into a `Long` number of milliseconds
(which can happen on the JVM), the resulting number is clamped to
`Long.MIN_VALUE` or `Long.MAX_VALUE`, depending on the sign.
When the opposite happens and the `Long` number of milliseconds is too big to
fit into an `Instant` (can happen on Native and JS), the `Instant` is clamped,
symmetrically. See the "Behavior on overflow" section for the rationale.
...
On overflow, the `Instant` is clamped to the platform-specific boundaries.
...
The idea is that when someone has years bigger than `1000000`, then surely
...
```kotlin
val breakfastEnded = Instant.parse("2024-08-20T07:34")
val lunchStarted = Instant.parse("2024-08-20T13:31")
...
the least / the biggest encountered value, but without exposing
